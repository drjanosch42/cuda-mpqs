...
| `--sieve_big_prime_start` | `<n>` | **v1.0.6, NARROW BATCH + `--params` only.** Overrides `gs_conf.bigPrimeStartIndex` (the GATHER/SCATTER factor-base split; otherwise `SB/32`). 0 = off; N > 0 must be **> 32** (`= midPrimeStartIndex`; at N ≤ 32 the mid-prime range inverts and the band is silently dropped). Power-of-two **not** required. Same four rejections as above | ⚠ **v1.0.7:** `loadPartialCustomConfig` sets `midPrimeStartIndex = 96` and `validateConfigs()` now rejects `midPrimeStartIndex > bigPrimeStartIndex`, so on this path the effective floor is **≥ 96** — values 33…95 still parse (the CLI predicate says > 32) but abort pre-sieve
| `--sieve_offsets_global` | -- | **v1.0.7**, default off, narrow only (forced off on wide). Keeps the GATHER `offsets1/offsets2/primes` arrays in global memory (one slice per block) instead of shared, so GATHER shared memory is `SB` (+ batch `B_values`) and `bPSI` no longer consumes the shared budget. Unmeasured as a production setting |
...
| `--params11` | `<p1,...,p11>` | **v1.0.7.** 11-field tuple `np,numIntervals,polyBlockSize,blocksPerCycle,metaGridDim,metaBlockDim,sasGridDim,sasBlockDim,sievingBlockSize,bigPrimeStartIndex,midPrimeStartIndex` — fields 1–8 are `--params`. Exactly 11 values required (parentheses stripped, empty values rejected; values past the 11th are ignored). A **0 field is unset** and keeps the loader derivation (`SB` from the smem budget, `numIntervals = 2M/SB`, `blocksPerCycle = numIntervals`, `bPSI = SB/32`, `midPS = 96`; the six grid/partition fields have no derivation — always set them). Loaded by `loadPartialCustomConfigDynamic()`, which also auto-sizes the bucket to the predicted peak unless `--bucket_size_factor` is given. Takes precedence over `--params`/autotune/history. ⚠ Unlike `--params` it skips the kernel-launch preflight, the LP `sasGridDim` floor and the legacy `sasBlockDim` cap. Also seeds `--param_test`. Not listed in `--help` |
...
| `--param_test` | **v1.0.7.** Tuning-complex parameter search (narrow path only). Not an `ExecutionMode`: it sets `MPQSConfig::param_test`, the run takes the `FULL_PIPELINE` route through Tuning [+ Autotune] and the search runs at the start of the sieve stage, prints the winner as a `--params11` line and **exits the process** (`exit(0)`; `exit(1)` if the seed is rejected). Seed = `--params11` if given, else `loadStandardConfig()`. Plain `--param_test` on a wide-path run logs an error and does not exit there |
| `--param_test_radius` | `<r>` — **v1.0.7**, default 3, must be ≥ 1. Maximum **face dimension** of the tuning complex swept by `--param_test` (1 = single-axis faces only, 3 = all faces); not a distance bound |
| `--param_test_legacy` | **v1.0.7 rename** of the ≤ v1.0.6 `--param_test`: exhaustive `Params8` grid search (`ExecutionMode::PARAM_TEST_LEGACY`, `runParamTestLegacy()`), exits after sieve init |
