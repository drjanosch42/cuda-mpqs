...
// Backward-compatible alias for the typo'd form (used by standalone benchmarks)
constexpr int LOG_STAGE_BW_INTIALIZATION           = LOG_STAGE_BW_INITIALIZATION;
...
struct LogConfig {
    ...
    // --- Backward-compat flat fields (deprecated; used by standalone test/benchmark init) ---
    // If sinks is empty, Init() constructs a default console sink from these fields.
    bool enable_cout        = true;
    bool enable_file        = false;
    bool csv_format         = false;
    int  min_severity_cout  = LOG_INFO;
    int  min_severity_file  = LOG_DEBUG_2;
    std::string file_path   = "bw_debug.log";
};
