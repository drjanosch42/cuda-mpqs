...
void HPCLogger::Init(const LogConfig& config) {
    ...
    // If sinks is empty, create a default console sink (backward compatibility)
    if (config_.sinks.empty()) {
        SinkConfig def;
        def.type           = SinkConfig::CONSOLE;
        def.min_severity   = LOG_INFO;
        def.max_severity   = LOG_RESULT;
        def.show_time      = true;
        def.show_stage     = true;
        def.show_module    = true;
        def.show_submodule = true;
        def.show_level     = true;
        def.wrap_width     = 120;
        active_sinks_.push_back({def, std::ofstream{}});
        return;
    }
